...
function routeStructuredData(pathname: string, title: string, description: string, url: string) {
  const base = { "@context": "https://schema.org", "@type": "WebPage", name: title, description, url, isPartOf: { "@type": "WebSite", name: SITE_NAME, url: SITE_URL } };
  if (pathname.startsWith("/products/")) {
    return [base, { "@context": "https://schema.org", "@type": "SoftwareApplication", name: title.split(" — ")[0].split(" | ")[0], description, applicationCategory: "BusinessApplication", url }];
  }
  return [base];
}
...
export function renderRouteHtml({ staticPath, pathname }: RenderOptions): string {
  ...
  const jsonLd = routeStructuredData(pathname, meta.title, meta.description, url).map((entry) => `<script type="application/ld+json">${safeJson(entry)}</script>`).join("");
  html = html.replace("</head>", `${jsonLd}\n</head>`);

  const noScript = `<noscript>${createNoScriptSummary(pathname, meta.title, meta.description)}</noscript>`;
  return html.replace(/<div id=["']root["']><\/div>/i, `${noScript}\n<div id="root"></div>`);
}

export function isKnownRoute(pathname: string): boolean {
  return SITE_ROUTES.some((route) => route.path === pathname);
}
