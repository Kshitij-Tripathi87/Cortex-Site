...
function isProductionStore(): boolean {
...
export async function recordFailedLogin(ip: string, maxAttempts: number, windowMs: number, lockoutMs: number): Promise<LoginAttemptState> {
  ...
  if (!supabase) {
    ...
    const state = !existing || now - existing.firstAttemptAt > windowMs
      ...
      : { ...existing, count: Math.min(existing.count + 1, maxAttempts) };
...
export async function pruneLoginAttempts(): Promise<void> {
  ...
  if (!supabase) {
    const now = Date.now();
    for (const [ip, state] of localLoginAttempts) {
      if (state.lockedUntil <= now && state.firstAttemptAt + 10 * 60 * 1000 <= now) localLoginAttempts.delete(ip);
    }
    return;
  }
  const { error } = await supabase
...
export async function consumeRateLimitBucket(key: string, max: number, windowMs: number): Promise<RateLimitBucket> {
  ...
  if (!supabase) {
    ...
    if (!current || current.resetAt <= now) bucket = { count: 1, resetAt: now + windowMs, allowed: true };
    else if (current.count >= max) bucket = { ...current, allowed: false };
    else bucket = { ...current, count: current.count + 1, allowed: true };
