...
adminRouter.post("/admin/auth/login", apiLimiters.adminLogin(), validateBody(SupabaseLoginSchema), async (req, res) => {
  ...
  try {
    ...
    await createSession(sessionId, Date.now() + SESSION_TTL_MS, getClientKey(req), req.get("user-agent") ?? "");
...
async function sessionIdentity(req: Request): Promise<{ ok: true } | { ok: false; status: number; error: string }> {
  const sessionId = readAdminSessionId(req);
...
adminRouter.get("/admin/session", async (req, res) => {
  ...
  if (!identity.ok) {
    clearAdminSessionCookie(res);
    res.status(identity.status).json({ error: identity.error, authenticated: false });
    return;
  }
  const sessionId = readAdminSessionId(req)!;
  setAdminSessionCookie(res, sessionId, SESSION_TTL_MS);
