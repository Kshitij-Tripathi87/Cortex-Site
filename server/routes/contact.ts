...
contactRouter.post("/contact", apiLimiters.write(), validateBody(ContactRequestSchema), async (req, res) => {
  ...
  const request = { id: newId("ct"), name: input.name, email: input.email, company: input.company, product: input.product ?? "", message: input.message, submittedAt: new Date().toISOString() };
  const hash = submissionHash(input.email, input.message, input.product ?? "");

  if (await isDuplicateContact(input.email, hash)) { res.status(200).json({ ok: true, duplicate: true }); return; }
  ...
  try {
    ...
    if (error instanceof Error && error.message === EMAIL_CONFIG_MISSING) console.warn("[contact] Notification configuration missing; submission remains persisted.");
    else console.error("[contact] Notification delivery failed; submission remains persisted:", error);
