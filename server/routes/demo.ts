...
demoRouter.post("/demo", apiLimiters.write(), validateBody(DemoRequestSchema), async (req, res) => {
  ...
  const request = {
    id: newId("dm"), name: input.name, email: input.email, company: input.company,
    role: input.role ?? "", companySize: input.companySize ?? "", product: input.product ?? "",
    message: input.message ?? "", preferredDate: input.preferredDate ?? "", submittedAt: new Date().toISOString(),
  };
