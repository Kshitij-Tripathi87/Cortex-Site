/* Issue #5: demo submissions persist before notification; retries are idempotent. */
...
demoRouter.post("/demo", apiLimiters.write(), validateBody(DemoRequestSchema), async (req, res) => {
  ...
  try {
    ...
    if (error instanceof Error && error.message === EMAIL_CONFIG_MISSING) {
      console.warn("[demo] Notification configuration missing; submission remains persisted.");
    } else {
